...
public class ArisChatHover extends JavaPlugin implements Listener, CommandExecutor {
    ...
    public void onChat(AsyncChatEvent event) {
        ...
        if (getConfig().getBoolean("ratelimit.enabled")) {
        ...
        if (getConfig().getBoolean("small-caps")) msgRaw = toSmallCaps(msgRaw);
        if (getConfig().getBoolean("antispam") && msgRaw.equalsIgnoreCase(lastMessage.get(uuid))) {
    ...
    private String handleMentions(String message) {
        String color = getConfig().getString("mention-color");
        for (Player online : Bukkit.getOnlinePlayers()) {
            String name = online.getName();
            if (message.contains(name)) {
                message = message.replace(name, color + name + "&r");
                soundManager.playSound(online, "msg-received");
            }
        }
        return message;
    }
